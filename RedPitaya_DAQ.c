    ...
    *ctx->gpio_reset = 1; usleep(1); *ctx->gpio_reset = 0;
    ...
    while (1) {
            ...
            if (n > 0) {
                ...
                if (strstr(cmd, "STOP")) {
                    if (record && log_file != NULL && buf_idx > 0) {
                        fwrite(chunk_buffer, 1, buf_idx * bytes_per_packet, log_file);
                        fflush(log_file);
                    }
                    free(chunk_buffer); free(frame_buffer);
                    return 0;
                }
        ...
        if (buf_idx >= CHUNK_SAMPLES) {
            ...
            // This is ideal because offline tools will be able to parse the .bin file exactly like a network stream.
            if (record && log_file != NULL) {
                for (int k = 0; k < buf_idx; k++) {
                    uint8_t *payload_ptr = chunk_buffer + (k * bytes_per_packet) + HEADER_SIZE;
                    fwrite(payload_ptr, 1, bytes_per_payload, log_file);
                }
            }
